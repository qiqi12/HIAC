...
def Plot(data, labels):
    ...
    color = [ '#001E6C','#FF5F00', '#40DFEF', '#F900DF', '#4D96FF', '#FFD93D', '#125B50', '#FF6363', '#CE49BF', '#22577E',   '#95CD41',
              '#8E3200',  '#C36A2D', '#B91646', '#4700D8']
    for i in [3, 2]:
        Together = []
        flag = 0
...
if __name__ == "__main__":
    ...
    global photoPath
    global sign
    filePath = "./Adj-3.txt"  # 改改改
    labelPath = ""
    with_label = True
    iterationTime = 6  # 改改改
    k = 150  # 改改改
    Tset = [0.8]  # 改改改
    threshold = 0#0.6902  # 改改改
    isIter = 1  # 改改改
        ...
        for T in Tset:
            ...
            for i in range(iterationTime):
                ...
                if i == iterationTime - 1:  # 得到改善数据集的聚类结果，通过与i比较大小，可以得到每次迭代后的聚类结果
                    ...
                    '''
                    sign = '(C)'
                    photoPath = "Agg-after-haic.png"
                    Plot(data_without_nml, result1.labels_)
        
                    sign = '(A)'
                    photoPath = "Kmeans-after-haic.png"
