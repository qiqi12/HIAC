...
def Plot(data, labels):
    ...
    color = ['#125B50', '#B91646', '#F900DF', '#4D96FF', '#40DFEF', '#FF6363', '#CE49BF', '#FFD93D', '#95CD41', '#4700D8', '#22577E',
             '#FF5F00',  '#8E3200', '#001E6C', '#C36A2D', ]  # 正常数据集
...
if __name__ == "__main__":
    ...
    global photoPath
    global sign
    filePath = "./t710k(nonoise).txt"  # 改改改
    labelPath = "./t710k(nonoise)-label.txt"
    with_label = 0  #data数据集里是否包含label
    iterationTime = 4  # 改改改
    k = 50  # 改改改
    Tset = [0.5]  # 改改改
    threshold = 1.26686 # 改改改
    isIter = 1  # 改改改
    ...
    if isIter:
        for T in Tset:
            ...
            if i == iterationTime - 1:  # 得到改善数据集的聚类结果，通过与i比较大小，可以得到每次迭代后的聚类结果
                ...
                sign = '(AGG)'
                photoPath = "./Agg-cluster_after_shrink_" + str(threshold) + "_" + str(k) + "_" + str(
                    T) + "_" + str(i + 1) + ".png"
                Plot(data_without_nml, result1.labels_)

                sign = '(KMeans)'
                photoPath = "./Kmeans-cluster_after_shrink_" + str(threshold) + "_" + str(k) + "_" + str(
                    T) + "_" + str(i + 1) + ".png"
