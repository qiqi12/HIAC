...
def Plot(data, labels):
    ...
    color = ['#125B50', '#4D96FF', '#FFD93D', '#FF6363', '#40DFEF', '#CE49BF', '#22577E', '#4700D8', '#F900DF', '#95CD41',
             '#FF5F00', '#40DFEF', '#001E6C', '#8E3200', '#001E6C', '#C36A2D', '#B91646']  # 正常数据集
    lineform = ['o']  # 噪声数据集、无需颜色代表类别的数据集
    for i in [1, 2, 6, 4, 5, 3, 7]:
        Together = []
        flag = 0
...
if __name__ == "__main__":
    ...
    global photoPath
    global sign
    filePath = "./Aggregation.txt"  # 改改改
    labelPath = ""
    iterationTime = 60  # 改改改
    k = 30  # 改改改
    Tset = [2.2]  # 改改改
    threshold = 1.19143  # 改改改
    isIter = 1  # 改改改
    ...
    if draw_clustering == 1:
        ...
        Plot(data, result_Agg.labels_ + 1)

        sign = '(B)'  # 每幅图上方的标号
        photoPath = "./original-KMeans.png"
        Plot(data, result_KMeans.labels_ + 1)
    ...
    if isIter:
        for T in Tset:
            ...
            if i == iterationTime - 1:  # 得到改善数据集的聚类结果
                ddataPCA = ddata.copy()
                ...
                Plot(data_without_nml, result1.labels_ + 1)
                sign = '(B)'
                photoPath = "./Kmeans-after-hiac.png"
                Plot(data_without_nml, result2.labels_ + 1)
            photoPath = str(threshold) + "_" + str(k) + "_" + str(T) + "_" + str(i + 1) + ".png"
            sign = '(F)'
            Plot(ddata, labels)
